...
exports.calculate_Size = function(folderId, folderPath, CallBack) {
  getSize(folderPath, (err, size) => {
    Folder_doc.findOneAndUpdate({_id: req.params.folderId }, {size, lastModifiedDate: Date.now}, {new:true}, function (err, folder) {
      if (err || !folder) return CallBack((err) ? err : "Folder Not Found", null);
